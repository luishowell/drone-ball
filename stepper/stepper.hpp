...
class Stepper {
  public:
    ...
    // accelerate or deccelerate the stepper, ramp time in s, speeds in rpm
    void ramp_speed(int start_speed, int end_speed, float ramp_time = 5.0, int speed_steps = 40); 

  private:
    void stepMotor(int this_step);

    //retuns step delay in us
    unsigned long calc_step_delay(long whatSpeed);
